 ...
 *
 */
angular.module('indeuApp').factory('GruppeModal', function($modal, $q) {

	var modal;
	var deferred;
	var local = this;

	local.modalInstance = ['$scope', 'ESPBA', 'Lookup', 'Const', 'Utils', 'SelectBrugerModal', 'DTOptionsBuilder', 'DTColumnBuilder', 'ImageUploadModal', 'Login', 'Log', 'group_id',
	function($scope, ESPBA, Lookup, Const, Utils, SelectBrugerModal, DTOptionsBuilder, DTColumnBuilder, ImageUploadModal, Login, Log, group_id) {

		$scope.accessLevels = Lookup.accessLevels();
		$scope.visibilityLevels = Lookup.visibilityLevels();

		$scope.__gruppeModal = {}; //should be sunsetted
		$scope.btnOk = group_id ? 'Gem og luk' : 'Opret gruppe og luk';
